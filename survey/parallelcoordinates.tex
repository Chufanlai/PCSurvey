...
视觉混杂问题是平行坐标研究的重点之一。要在有限的空间里表现大量的图元，主要有三种思路：表现密度分布、减少图元数目、以及扩大显示空间。
...
然而在平行坐标里，对绘制结果作密度估计并不反映数据域的密度。Zhou等人~\citep{zhou2009splatting}提出用抛雪球算法来检测数据聚类，并逐步显现其密度分布（如图~\ref{fig:PC_Design_Density2}）。
...
进一步地，连续型平行坐标的相关研究~\citep{heinrich2009continuous}~\citep{lehmann2011features}~\citep{feng2010matching}~\citep{heinrich2011progressive}揭示了数据域里的密度估计及相应的绘制方式（见图~\ref{fig:PC_Data_Continuous2}、图~\ref{fig:PC_Data_Uncertainty}）。如此一来，既无需费时绘制所有数据，避免了混杂问题，又能较好地保留数据的信息。
