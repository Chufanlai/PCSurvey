...
当数据量较大时，直接绘制的效率较低，容易导致交互过程的延迟，影响用户体验。Rosenbaum等人~\citep{rosenbaum2012progressive}提出建立数据的精度层级（Data Hierarchy），通过低精度小规模的数据来提供概要信息，以平衡信息量与绘制效率（如图~\ref{fig:PC_Data_Splatting}）。但在视图精度不断提高的过程中，每个层级的数据都需要重新获取与绘制， 其总体效率会比直接绘制更低。相比于这种自顶向下的方式，Heinrich等人~\citep{heinrich2011progressive}通过数据域的再采样，自底向上地构建连续型平行坐标。随着样本的不断交叠，用户可以逐步了解数据域的分布特点。
