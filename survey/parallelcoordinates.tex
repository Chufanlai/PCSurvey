...
然而很多时候，即使是经过概括的视图，也会因为较高的数据异质性而显得杂乱。因此可以先进行聚类、从根源上消除混杂，亦即聚类表达型的方法。

%McDonnell等人~\citep{mcdonnell2008illustrative}基于数据聚类的结果，用条带和阴影分别表现每个聚类的走势和密度（如图~\ref{fig:PC_Design_Abstraction1}）。这种形式既具概括性又不乏细节，适合用作数据展示，但既有的聚类结果容易主导用户的理解。

%\begin{figure}[!htb]
%\centering
%\includegraphics[width=0.9\linewidth]{images/PC_Design_Abstraction2.eps}
%\caption{\label{fig:PC_Design_Abstraction1}利用不透明度或色调表现折线密度}
%\end{figure}
