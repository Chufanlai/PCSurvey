...
针对前一节中提出的各种问题，领域学者们做了许多研究工作来对平行坐标进行改进和延展。这一节将详细地回顾这些工作，并对它们进行比较、总结。为了保持行文结构、方便读者对照理解，这一节依然按数据、视图、感知和交互四个部分来展开。
...
正如~\ref{subsection:designBasics}节所述，由于折线重叠问题，平行坐标并不适合表现类别型数据。在这种数据里，大量样本具有相同的取值，它们相互之间无法被区分。但事实上，这些数据的分析重点并不在于单个样本，而是同一类样本的集合，因此需要在平行坐标里加入表达集合关系的元素。Kosara~等人利用线段宽度来映射取值相同的样本的数量，提出了平行集合的形式（如图\ref{fig:PC_Data_ParallelSet}）。每个轴上的不同取值相当于不同的集合，相应条带的宽度则映射了集合的大小。

\begin{figure}[!htb]
\centering
\includegraphics[width=0.9\linewidth]{images/PC_Data_ParallelSet.eps}
\caption{\label{fig:PC_Data_ParallelSet}平行集合的构造。本图来源于Kosara~等人的论文~\citep{kosara2006parallel}~。
}
\end{figure}
